  ...
  switch (inst.getOpcode())
  {
    ...
    case llvm::Instruction::SDiv: {
        ...
        if(llvm::ConstantInt *constInt = llvm::dyn_cast<llvm::ConstantInt>(op1)){
          ...
          instr_list->Append(new assem::OperInstr(
            "idivq `s0", 
            new temp::TempList({rax, rdx}), new temp::TempList({src2_temp, rax, rdx}), nullptr));
          //step4 将商储存到dest中
           instr_list->Append(new assem::MoveInstr(
          ...
          instr_list->Append(new assem::OperInstr(
            "idivq `s0", 
            new temp::TempList({rax, rdx}),  new temp::TempList({temp_reg, rax, rdx}), nullptr));
          instr_list->Append(new assem::MoveInstr(
            "movq %rax, `d0", new temp::TempList(dest_temp), new temp::TempList(rax)));
        } else {
          //dest = src1 / src2
          ...
          instr_list->Append(new assem::OperInstr(
            ...
            nullptr));
          instr_list->Append(new assem::OperInstr(
              "idivq `s0",  new temp::TempList({rax, rdx}), new temp::TempList({src2_temp, rax, rdx}), nullptr));
          instr_list->Append(new assem::MoveInstr(
            "movq %rax, `d0", new temp::TempList(dest_temp), new temp::TempList(rax)));
        }
        break;
